...
// div.style.color = 'red';
div.innerText = 'Some new text';
document.body.appendChild(div)
document.body.appendChild(div.cloneNode(true))
//
// - Є масив:
// Зробити ul в середині якої будуть лежати елементи масиву (кожен в своєму li)
const arrItems = ['Main', 'Products', 'About us', 'Contacts']
...
for (const element of coursesAndDurationArray) {
    let div = document.createElement('div');
    div.innerText = `${element.title} , month duration - ${element.monthDuration}`
    document.body.appendChild(div);
}
//
// =========================
...
for (const element of coursesAndDurationArray1) {
    ...
    p.innerText = `${element.monthDuration} month duration`;
    div.append(h1, p)
}
...
divContainer.classList.add('div-wrap');
...
for (const member of simpsons) {
    ...
    h2.innerText = `${member.name} ${member.surname} , age - ${member.age}`;
    let p = document.createElement('p');
...
for (const course of coursesArray) {
    ...
    duration.innerText = `Duration - ${course.monthDuration} month, ${course.hourDuration} hour`
    courseBox.appendChild(duration)

    let modules = document.createElement('ul')
    courseBox.appendChild(modules);
